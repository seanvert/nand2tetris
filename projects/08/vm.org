...
- preciso colocar alguma coisa aqui pra mexer com vários arquivos e
  soltar um só. 
...
#+RESULTS:
: stdIn:453.5-478.5 Warning: match nonexhaustive
:           (x :: xs,outstream,n) => ...
:   
: val readFileList = fn : string list -> TextIO.outstream -> int -> unit
